        ...
        $total_bill    = 0;
            ...
            if (is_array($listData) && !empty($listData)) {
                foreach ($listData as $lap) {
            		...
            		$worksheet->write($i + 5, 0, $i, $format_center_data);
                    $worksheet->write($i + 5, 1, $lap['nama_technical_provider'], $format_center_data);
                    $worksheet->write($i + 5, 2, $lap['nama_mitra'], $format_center_data);
                    $worksheet->write($i + 5, 3, $lap['product'], $format_center_data);

                    $worksheet->writeNumber($i + 5, 4, intval($nilaiLembar), $format_lembar_data);
                    $worksheet->writeNumber($i + 5, 5, floatval($nilaiTagihan), $format_uang);
                    $worksheet->writeNumber($i + 5, 6, floatval($nilaiAdmin), $format_uang);
                    $worksheet->writeNumber($i + 5, 7, floatval($nilaiTotal), $format_uang);
